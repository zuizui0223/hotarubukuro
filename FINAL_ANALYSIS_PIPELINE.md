...
flowchart TD
    ...
    D --> E["V17 局所Bombus群集転換<br/>25 km近距離ペア"]
    D --> F["V20 局所有色孤立点<br/>環境類似白花近傍"]
    F --> G["V21 5–50 km人口・土地利用"]
    G --> H["V22 DID近接性感度解析"]
    E --> I["送粉者群集との局所対応"]
...
V16のcross-fitted環境＋INLA-SPDEモデルを自然基準とする。
...
V17の園芸仮説とは独立な局所ペア部分を、最終的な送粉者解析として
採用する。
...
- `final.Rmd`内の古い埋め込み結果
- 全花連続a*を一応答にした解析
- 白花a*をアントシアニン量とみなす解釈
- V16の旧園芸tier解析
- V18–V19の極端値random forestを最終証拠とする解析
- V20の旧matched-control園芸診断と早咲・濃色収束検定
- 残差を目的変数にした二段階回帰
- 5種Bombusの独立係数解釈
- 送粉者の因果的選択圧、人為的植栽、園芸由来、逸出、交雑、
  遺伝子汚染の断定

V19はV20–V21が利用する応答盲検の景観特徴量作成にだけ残し、その
極端値推論は採用しない。
...
既存のV16チェックポイントからV17–V22を再生成する通常実行：

```powershell
& 'C:\Program Files\R\R-4.5.3\bin\Rscript.exe' `
  scripts/run_final_analysis_pipeline_v1.R `
...
V15の固定セル表からV16と特徴量作成も再実行する場合は
`--mode=full`を使う。画像抽出、V11、V15は手動確認済みの固定上流入力
として監査する。
