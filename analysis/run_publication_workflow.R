...
suppressPackageStartupMessages({
  library(terra); library(sf); library(lavaan); library(INLA); library(qgam); library(yaml)
})
source("R/pigmentation_workflow.R")
set.seed(42); INLA::inla.setOption(num.threads = "1:1")
...
valid <- !identity_bad & nonempty(d$observation_id) & complete.cases(d[c("date", "latitude", "longitude", "R", "G", "B")])
...
d <- d[valid, , drop = FALSE]; reviewer_contract(d)
d <- cbind(d, srgb_to_lab(d$R, d$G, d$B))
...
# 2. Legacy environmental domains; CHELSA CMI replaces only the unavailable AI layer.
env_files <- c(
...
# 3. Potential Bombus availability; total is primary, distribution types are sensitivity diagnostics.
...
write.csv(cor(d[c("Bombus_availability","Bombus_widespread","Bombus_montane","elevation","Temperature_PC1","precip_PC1","topo_PC1")],use="pairwise.complete.obs"),file.path(out_dir,"bombus_environment_correlations.csv"))

# 4. One integrated natural model. Shared gradients are not reported as pure causal partitions.
natural_terms <- integrated_model_terms("Bombus_availability")
common <- c("Pigment","longitude","latitude",natural_terms,"Bombus_widespread","Bombus_montane")
...
dm <- d[cc,,drop=FALSE]
for (nm in c(natural_terms,"Bombus_widespread","Bombus_montane")) dm[[paste0("z_",nm)]] <- z(dm[[nm]])
dm$y <- z(dm$Pigment); z_terms <- paste0("z_",natural_terms)
...
fit_spde <- function(label,terms){
  ...
  stk <- inla.stack(data=list(y=dm$y),A=list(A,1),effects=list(spatial=seq_len(spde$n.spde),fixed=fixed),tag="est")
  form <- as.formula(paste("y ~",paste(c("0 + Intercept",terms,"f(spatial, model = spde)"),collapse=" + ")))
  fit <- inla(form,family="gaussian",data=inla.stack.data(stk),control.predictor=list(A=inla.stack.A(stk),compute=TRUE),control.compute=list(waic=TRUE,dic=TRUE,cpo=TRUE,config=TRUE),verbose=FALSE)
...
comparison <- do.call(rbind,lapply(fits,function(x){cpo<-x$fit$cpo$cpo;good<-is.finite(cpo)&cpo>0;data.frame(model=x$label,n=nrow(dm),mesh_vertices=mesh$n,WAIC=x$fit$waic$waic,DIC=x$fit$dic$dic,mean_neglogCPO=mean(-log(cpo[good])),failed_CPO=sum(!good))}))
comparison$delta_WAIC <- comparison$WAIC-min(comparison$WAIC); write_table(comparison,"integrated_spde_model_diagnostics.csv")
...
yhat <- fits$total$fit$summary.fitted.values$mean[idx]; yhat_sd <- fits$total$fit$summary.fitted.values$sd[idx]
dm$predicted_pigment_z <- yhat; dm$predicted_sd <- yhat_sd; dm$resid_raw <- dm$y-yhat; dm$resid_zfit <- dm$resid_raw/pmax(yhat_sd,1e-6)
...
qsummary <- do.call(rbind,lapply(seq_along(qfit$models),function(i){sm<-summary(qfit$models[[i]]);data.frame(quantile=qfit$taus[[i]],deviance_explained=sm$dev.expl,n=nrow(qfit$data),formula=paste(deparse(qfit$formula),collapse=" "))}))
write_table(qsummary,"qgam_model_summary.csv")
cut <- quantile(dm$resid_zfit,0.95,na.rm=TRUE)
candidates <- dm[is.finite(dm$resid_zfit)&dm$resid_zfit>=cut,intersect(c("observation_id","source_row","url","date","DOY","latitude","longitude","Pigment","predicted_pigment_z","resid_zfit",human_terms,"elevation","landscape_setting","image_sha256","qc_flags"),names(dm)),drop=FALSE]
candidates$horticultural_status <- "unreviewed_candidate_only"; write_table(candidates[order(-candidates$resid_zfit),],"anthropogenic_outlier_review.csv")
write_table(dm,"analysis_data_with_predictions.csv"); writeLines(capture.output(sessionInfo()),file.path(out_dir,"sessionInfo.txt"))
