...
- `method-development/` — obsolete analysis estimands and code paths, including the old five-species Bombus limitation gate, relaxation/local-contrast variants, old publication orchestrator and superseded local-turnover implementation.
...
The project involved genuine scientific iteration: alternative pollinator estimands, scale choices, candidate definitions and manuscript architectures were tested and then rejected or demoted when they did not answer the final ecological question cleanly. Keeping those files makes the history auditable without making readers guess which result is current.
