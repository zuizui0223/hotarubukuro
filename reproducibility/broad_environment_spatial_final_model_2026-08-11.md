...
## Collinearity policy and exact VIF audit

VIF is treated as a diagnostic rather than a universal binary validity rule. The final policy is:

- **VIF <5:** preferred;
- **VIF 5–10:** acceptable only with explicit focal-term, coefficient/spatial and geographically blocked transfer stability;
- **VIF >10:** do not promote an environmental expansion without exceptional mechanistic and predictive justification.

This avoids treating either 5 or 10 as an automatic deletion threshold. It is especially important in the current hierarchical interaction model, where all constituent main effects must remain when their product is fitted and where VIF among fixed predictors does not itself diagnose spatial confounding with the latent field.

Exact values from the frozen analysis population are:

### State

- maximum VIF: Soil PC1 = **4.430**;
- Temperature PC1 = 4.146;
- all remaining fixed terms <2.62.

Thus the final state model passes even a hard VIF <5 convention.

### Intensity

With East/West and the retained thermal interaction:

- Temperature PC1 = **6.340**;
- Soil PC1 = **5.233**;
- Temperature PC1 × temperature seasonality = **1.664**;
- East/West = 1.822.

The elevated maximum is generated by shared geography among constituent environmental axes rather than by the retained interaction. In the pigmented subset, Temperature PC1 correlates approximately -0.731 with Soil PC1 and -0.673 with temperature seasonality; the focal interaction remains only moderately correlated with its constituent axes.

Removing East/West lowers the maximum VIF to 4.885 and the interaction VIF to 1.585. That model is not adopted because the spatial audit showed about 5.9 units worse WAIC and no robust blocked-transfer advantage. The regional adjustment is therefore not removed merely to make a diagnostic cross the value five.

The rejected VPD expansions had VIFs around 25.8–26.0 and failed geographical-transfer criteria, so they are rejected under either a five- or ten-based convention.
