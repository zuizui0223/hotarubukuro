...
output_dir <- arg_value(
  "--output",
  "results/broad_supported_environment_distance"
)
...
# The final intensity model used a standardized Temperature PC1 × temperature-
# seasonality term. Reproduce that fixed transformation from the frozen
# observation table; do not refit the final model or weight distances by its
# coefficients.
...
observations$int_thermal_variability <-
  (interaction_reference - interaction_centre) / interaction_spread
...
for (response_name in names(supported_terms)) {
  ...
  if (any(!stats::complete.cases(cells[terms]))) {
...
sample_space_null <- function(train, test, response, family, mesh,
  ...
  if (family == "binomial") {
    ...
    simulated <- vapply(seq_len(n_samples), function(k) {
      stats::rbinom(
        nrow(test), size = trials_test, prob = probability[, k]
      ) / trials_test
    }, numeric(nrow(test)))
    ...
    simulated <- vapply(seq_len(n_samples), function(k) {
      stats::rnorm(
        nrow(test), mean = eta[, k], sd = sqrt(1 / precision[[k]])
      )
    }, numeric(nrow(test)))
...
contrast_from_groups <- function(observed_div, null_div, geo_bin, env_dist,
  ...
  for (bin in sort(unique(geo_bin))) {
    idx <- which(
      geo_bin == bin & is.finite(env_dist) & is.finite(observed_div)
    )
    ...
    rows[[length(rows) + 1L]] <- data.frame(
      ...
      environment_low_cut = cuts[[1L]],
      environment_high_cut = cuts[[2L]],
...
utils::write.csv(
  primary_table,
  file.path(output_dir, "primary_supported_environment_distance_test.csv"),
  row.names = FALSE
)
utils::write.csv(
  stratum_table,
...
utils::write.csv(
  pair_table,
  file.path(output_dir, "heldout_pair_supported_environment_distance.csv"),
  row.names = FALSE
)
utils::write.csv(
  heldout_table,
...
utils::write.csv(
  term_table,
  file.path(output_dir, "supported_environment_terms.csv"),
  row.names = FALSE
)

metadata <- data.frame(
  field = c(
    "analysis", "observations", "cells", "final_model_refit",
    "space_null", "spatial_folds", "posterior_samples", "seed",
    "max_pairs_per_fold", "geographic_distance_bins",
    "pigmentation_state_terms", "conditional_intensity_terms",
    "environment_distance", "primary_estimand", "claim_boundary"
  ),
  value = c(
