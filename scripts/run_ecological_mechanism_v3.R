...
script_path <- if (length(file_arg)) {
  sub("^--file=", "", file_arg[1])
} else "scripts/run_ecological_mechanism_v3.R"
...
if (!nzchar(occurrence_dir) || !dir.exists(occurrence_dir)) {
  stop(
    ...
    "It must contain the five frozen *_gbif.csv files produced by final.Rmd.",
    call. = FALSE
  )
}

run_ecological_mechanism_v3(
...
cat("Completed ecological mechanism v3: ", normalizePath(output_dir, winslash = "/"), "\n", sep = "")
