...
source("scripts/predictive_replication_v9.R")

dir.create(output_dir, recursive = TRUE, showWarnings = FALSE)
checkpoint_dir <- file.path(output_dir, "checkpoints")
dir.create(checkpoint_dir, recursive = TRUE, showWarnings = FALSE)
final_rmd_md5_before <- if (file.exists("final.Rmd")) {
...
metadata <- data.frame(
  field = c(
    ...
    "final_Rmd_modified_by_v16", "final_Rmd_md5"
  ),
  value = c(
...
writeLines(
  c(
    ...
    "",
    "`final.Rmd` is intentionally not modified by this pipeline."
  ),
