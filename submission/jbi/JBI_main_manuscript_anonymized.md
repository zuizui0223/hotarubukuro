...
**Methods:** We converted author-screened YAMAP hiking photographs into a national two-part phenotype: pigmentation state and pigmented-only intensity. INLA-SPDE models separated measured environmental associations from continuous residual geography. We moved an environment-derived bumblebee hypothesis to 67 independently fixed white-pigmented boundaries, then replayed an ecological departure detector on 10,000 natural predictive maps before examining human context.

**Results:** The curated dataset contained 1,922 observations; the matched YAMAP stream contained 3.81 times as many focal-species records as iNaturalist. Pigmentation state and intensity showed different geography. Pigmentation was less likely in warmer climates, whereas intensity depended on Temperature PC1 × temperature seasonality and was lower in wetter and more rugged environments. Residual spatial ranges also differed between responses. Changing scale isolated a small positive mean bumblebee contrast at 5 km from misleading highland overlap, although the signal was heterogeneous and scale-sensitive. Predictive replay converted 16 apparent departures into calibrated field targets: their frequency was compatible with natural maps. Population exposure within 5 km was the leading post-selection human feature (global maxT FWER P=0.0548).

**Main conclusions:** The study resolves one flower-colour map into distinct environmental, spatial, local biotic and provenance hypotheses. Its contribution is both empirical and conceptual: a new national trait dataset reveals response-specific ecology, while changing comparison scale separates signals that a single national regression would confound. The resulting model links spatially varying physiological and reproductive value, history and occasional human movement to polymorphism maintenance, and identifies where each mechanism can be tested directly.
...
The local pollinator test has a strong natural-history basis in *C. punctata*. Bumblebees are effective pollinators of its tubular flowers, and geographical changes in *Bombus* fauna are linked to pollen-removal ecology and breeding-system change (Inoue & Amano, 1986; Inoue, 1988; Nagano et al., 2014). Rather than asking whether national maps overlap, we asked whether predicted habitat opportunity for focal bumblebees increases specifically from the white side to the pigmented side of abrupt nearby boundaries. This scale shift turns an ambiguous broad overlap into a directional local comparison without pretending that an SDM measures visits or selection.
...
### 3.3 Changing scale separated shared mountain geography from a local Bombus signal

The 5-km design produced 67 sharp transitions between pure white and pigmented cells, with median separation 2.0 km. The fixed pairs were environmentally closer than ordinary local edges: median eight-axis distance was 0.244 versus 0.318. The local design therefore compared nearby colour states in a substantially tighter environmental neighbourhood than a national overlay.

Mean focal-bumblebee support was 0.0359 higher on the pigmented side (one-sided sign-flip P=0.0272). The effect was heterogeneous: median contrast was -0.0028, 49.3% of pairs were positive and the 5-, 10- and 25-km family gave q=0.0815. Mean contrast fell to +0.0084 at 10 km (P=0.325) and +0.0029 at 25 km (P=0.436), and raw SDM support did not reproduce the 5-km result (P=0.267).

The same scale-aware analysis also separated an attractive false lead from the focal result. Pigmented highland flowers overlapped montane/alpine *Bombus* support nationally, but the contrast vanished when nearby endpoints were constrained to similar elevation (all one-sided P>=0.755 for the <=50 m test). No persuasive Bombus relationship was found for intensity among pigmented flowers. The pollinator stage therefore achieved a sharper inference than either “effect” or “no effect”: broad highland overlap reflected shared geography, while the remaining plausible signal was small, local and associated with the white-pigmented state boundary.
...
### 4.3 Changing scale localized the pollinator hypothesis

The pollinator analysis demonstrates why scale was part of the biological hypothesis. A national overlay would have produced an appealing association between highland bumblebees and pigmented flowers. The equal-elevation comparison showed that this pattern belonged to shared mountain geography. The local boundary design then asked a more discriminating question and found a small mean focal-Bombus contrast within environmentally tighter comparisons. Together, these results separate two signals that a national model could easily merge: broad abiotic co-distribution and a possible local biotic contribution.

The local result is heterogeneous, but that heterogeneity refines rather than nullifies the hypothesis. Its restriction to 5-km white-pigmented boundaries, attenuation with distance and absence along the intensity axis are more consistent with local maintenance or loss of a visible pigmented state than with progressive darkening across Japan. This is a new ecological conclusion from the current data: if bumblebees contribute, their likely role is spatially limited and state-specific.

The SDMs measure habitat opportunity, so realized selection remains the next causal layer. Crucially, the analysis now specifies where and what to measure. The 67 boundaries are field locations for species-resolved visitation, bee visual contrast, stigma contact, pollen deposition, seed set and selection gradients. The macro analysis has therefore converted a broad correlational hypothesis into a tractable set of local tests.
...
The broader biogeographic contribution lies in this resolution. Repurposed images revealed the phenotype; broad models separated measured environment from coherent residual geography; a scale shift distinguished shared mountain overlap from a local pollinator signal; predictive replay turned apparent exceptions into calibrated targets; and post-selection analysis identified the leading human-context clue. The study does not obtain clarity by showing only convenient results. It obtains clarity by letting positive, weak and null results perform different inferential jobs within one designed sequence. The payoff is a reproducible map-to-mechanism programme—and a testable explanation for how flower-colour polymorphism can persist across a species range.
