...
**Methods:** We converted author-screened YAMAP hiking photographs into a national two-part phenotype: pigmentation state and pigmented-only intensity. INLA-SPDE models separated measured environmental associations from continuous residual geography. We characterized 67 independently fixed white-pigmented boundaries within 5 km as the local bumblebee test, then applied a separate isolate-event detector to the observed map and replayed it on 10,000 natural predictive maps before examining human context.

**Results:** The curated dataset contained 1,922 observations; the matched YAMAP stream contained 3.81 times as many focal-species records as iNaturalist. Pigmentation state and intensity differed geographically; only state exceeded a cross-fitted spatial expectation along environmental difference. Pigmentation was less likely in warmer climates, whereas intensity depended on Temperature PC1 × temperature seasonality and was lower in wetter and more rugged environments. At local boundaries, the mean focal-bumblebee contrast favoured pigmented sides, but the near-zero median, 49.3% positive pairs and broader-scale attenuation showed that a subset of boundaries drove this correspondence. A separate equal-elevation guardrail showed that stronger-looking highland overlap was shared mountain geography. The observed event rule selected 16 locally isolated pigmented cells, but replay on natural maps reproduced that event frequency. Population exposure within 5 km was the leading post-selection human feature (global maxT FWER P=0.0548).

**Main conclusions:** The study resolves one flower-colour map into distinct environmental, spatial, locally heterogeneous biotic and provenance hypotheses. Quantitative phenotyping revealed that the environmentally ordered white-pigmented switch is not equivalent to variation within the pigmented state. Changing comparison unit exposed a subset-driven local Bombus correspondence while preventing national highland overlap from being mistaken for mechanism. Natural-map replay then separated reproducible field targets from evidence that an additional process was required. The resulting model links spatially varying physiological and reproductive value, unresolved geography and possible contemporary movement to polymorphism maintenance, while identifying where each mechanism can be tested directly.
