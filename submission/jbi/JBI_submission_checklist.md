...
- The conceptual advance is **scale-matched attribution**: new trait data establish the national pattern; INLA-SPDE separates measured environment from coherent residual geography; the environment-derived Bombus hypothesis is moved to local boundaries; and apparent departures are calibrated against natural maps before human context is read.
...
- Main-text target: <=6,000 words.
  - Current Introduction-through-Discussion count = **3,879 words** by repository validator.
...
- Main presents the positive scientific output of the scale change: shared highland geography is distinguished from a small, state-specific local focal-Bombus signal.
...
- **Bombus killing part:** scale change separates misleading mountain overlap from a small local state-boundary signal and identifies 67 field sites for direct pollination tests.
...
- The final paragraph states that the study uses positive, heterogeneous and null results for different inferential jobs rather than displaying only convenient results.
- Five-species community turnover remains Supporting Information.
- Main Bombus result reports its median, sign proportion, scale attenuation and raw-support result alongside the positive local mean contrast.
