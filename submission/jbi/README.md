...
- `JBI_main_figure_captions.md` — submission-facing captions for the four current Main figures;
- `figures/README.md` — generated-figure policy, filenames and artifact boundary;
- `JBI_supporting_information_outline.md` — six-appendix SI architecture;
- `JBI_sdm_model_building_checklist.md` — current five-species SDM decisions and reproducibility checklist;
...
- `bundle/README.md` — generated editable delivery-bundle boundary and readiness semantics;
...
The binary figure files are generated by `.github/workflows/jbi-main-figures.yml` from checksum-locked manuscript evidence and uploaded as an Actions artifact. They are not committed as opaque binaries. Each successful bundle contains four 600-dpi PNG review copies, four vector PDFs, per-file SHA-256 hashes, a source manifest and a numerical lock.

The editable delivery package is generated by `.github/workflows/jbi-submission-bundle.yml`. It combines the anonymized Main manuscript with embedded Figures 1–4, supplies one combined editable Supporting Information DOCX containing Appendices S1–S6, and produces separate title-page, cover-letter, translated-abstract and SDM-checklist DOCX files. All generated Word files are converted through LibreOffice in CI. The bundle deliberately remains `portal_ready=false` until author-controlled identifying, disclosure, private-repository-link and taxon-image requirements are completed.
