...
The current source reconstruction began with 1,965 eligible source records. One later exact-image hash copy was removed under stable source-row ordering, and 42 additional records did not enter the reconstructed two-part phenotype/model boundary. The final observation-level analysis contained:
...
The temperature-axis interval excluded zero, with lower pigmentation probability toward the warmer end of the fitted axis. Other fixed-effect intervals included zero after conditioning on the spatial field.
...
Conditional intensity declined toward the warmer end of Temperature PC1 and toward the rugged/steep end of Topography PC1. The East/West term also differed after conditioning on environment and the spatial field, but it remains an unresolved regional adjustment rather than a process attribution.
...
The pigmentation-state field therefore operated over a broader fitted scale than the conditional-intensity field. These are model-based correlation ranges, not dispersal distances or dated historical boundaries.
...
The broad-model claim is limited to a strong geographical reference containing measured environmental associations and substantial unresolved spatial structure. It is not a variance partition, causal environmental decomposition, pollinator-selection estimate or population-history reconstruction.
