...
1. **Directional Main test:** are the pigmented sides of the sharpest nearby white-pigmented boundaries associated with greater predicted availability of the documented broad focal pollinators *Bombus ardens* and *B. diversus*?
2. **Exposure and design robustness:** does that directional result persist when spatial scale, transition threshold, species set or raw SDM scale changes?
...
Environment was not used to select, orient or weight pairs. It is used only as a confounding diagnostic. To align that diagnostic with the finalized Broad pigmentation-state analysis, the current primary diagnostic measures root-mean-square distance across the same eight standardized abiotic axes: Temperature PC1, precipitation PC1, temperature seasonality, precipitation seasonality, topography PC1, soil PC1, soil PC2 and RSDS. The historical broad/within-50-km four-PC distance is retained as a sensitivity/provenance diagnostic.
...
The directional mean attenuated rapidly beyond the strict local window. Under the finalized Broad axes, selected pure transitions were less environmentally separated than the full local edge set at all three radii. The historical four-PC diagnostic showed the same ordering (0.180 versus 0.343 at 5 km; 0.265 versus 0.442 at 10 km; 0.371 versus 0.520 at 25 km). The conclusion is therefore not an artefact of which of these two defensible environmental summaries is used for the descriptive balance check.
...
Raw cloglog support was retained to show whether the result depended on species-specific occurrence-reference scaling. The raw maximum of *B. ardens* and *B. diversus* did not reproduce the focal nominal result.
...
The fixed local inferential family does not contain separate species-by-species hypothesis tests. Individual occurrence-referenced scores remain available in the support table for transparent inspection, but the current biological estimands are the documented two-species guild, the all-five maximum and the montane/alpine guardrail. This avoids selecting a species after inspecting the direction of its local association.

The raw-support failure, all-five P=0.0836, near-zero median, q=0.0815 across the three pure-transition scales and broader-scale null results are part of the Main claim ceiling, not ancillary details to be omitted.
...
**Table S5.6. Spatial repetition under the primary match specification.**
...
The visually striking national overlap between pigmented highland flowers and *B. beaticola*, *B. consobrinus* and *B. honshuensis* was tested by restricting the established pure transitions to endpoints with nearly equal elevation. The signed statistic was the pigmented-minus-white difference in the maximum occurrence-referenced support across the three montane/alpine taxa.
...
The appropriate interpretation is therefore shared high-elevation/spatial co-distribution, not substitution by montane bumblebees and not an additional pollinator mechanism.

## Main-versus-Supporting interpretation

The current manuscript keeps the following inferential separation:

- **Main 2:** a weak, highly local, directionally signed correspondence between pigmentation state and predicted availability of *B. ardens* plus *B. diversus*;
- **current-Broad environmental guardrail:** the fixed transition pairs are also locally close in the finalized eight-axis abiotic state space, but this does not eliminate residual fine-scale environmental confounding;
- **Supporting biogeography:** sharp colour boundaries also tend to coincide with predicted five-species community boundaries under several local matching designs;
- **negative guardrail:** the montane/alpine overlap vanishes when elevation is held approximately equal.

The community result does not strengthen the focal mean into causal evidence because it is unsigned and generated from environment-derived surfaces. The montane result prevents a visually attractive map overlap from being narrated as a second positive mechanism. The final-eight-axis distance audit prevents the local-comparison rationale from depending on an outdated environmental representation, while leaving the independently frozen Bombus pair test unchanged.
...
- `analysis_sensitivity/audit_bombus_final8_environment_distance.R` — final-eight-axis environmental balance audit of the already fixed transition pairs;
- `.github/workflows/bombus-final8-environment-audit.yml` — checksum-locked audit workflow;
...
The frozen local-transition evidence is workflow `31263324505`, artifact `9023416810`, SHA-256 `3f7ac07ea90e2b732441a9f80a38ea49871014722d008769370524b947007e34`. The frozen spatial-replication evidence is workflow `31285234317`, artifact `9029595037`, SHA-256 `067dd3408b2a7f046ba263732ffa4cefa2f54a7f1fb672478be46bca425f6bf0`.

The current-Broad environmental-distance audit is workflow `31538548679`, artifact `9119773035`, SHA-256 `51fc846d2f3d815d8bbf3c0b95647eabeb0acf731f1e6dd164c11a0dfe1b896f`. Its focal 5-km result was selected-pair median final-eight-axis RMS distance 0.2441 versus 0.3175 across all local edges (ratio 0.7687); the same ordering held at 10 and 25 km.
