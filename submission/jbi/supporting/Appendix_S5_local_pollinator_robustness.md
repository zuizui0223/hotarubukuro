...
A greedy non-overlapping pair set was selected by decreasing absolute colour difference, then shorter geographic distance, then stable cell identifier. Pair selection was blind to *Bombus* values and to white-to-pigmented orientation. Only after the set was fixed was each pair oriented white -> pigmented.
...
Environment was not used to select or orient pairs. As a descriptive diagnostic, each edge was measured in the predefined four-dimensional environmental-PC space (broad and within-50-km PC1 and PC2).
...
| Median environmental distance, selected transitions | 0.180 |
| Median environmental distance, all local edges | 0.343 |
...
**Table S5.2. Occurrence-referenced focal exposure across scales for pure transitions.**

| Radius | Pairs | Mean contrast | Median contrast | Proportion positive | One-sided P |
|---:|---:|---:|---:|---:|---:|
| 5 km | 67 | +0.03590 | -0.00277 | 0.493 | 0.02716 |
| 10 km | 109 | +0.00840 | +0.00538 | 0.514 | 0.32454 |
| 25 km | 171 | +0.00292 | -0.00179 | 0.497 | 0.43575 |

The directional mean attenuated rapidly beyond the strict local window. The selected pairs were less environmentally separated than the full local edge set at all three radii (0.180 versus 0.343 at 5 km; 0.265 versus 0.442 at 10 km; 0.371 versus 0.520 at 25 km). This supports the local-comparison rationale but does not show that environmental confounding has been eliminated.
...
The raw-support failure, all-five P=0.0836, near-zero median and broader-scale null results are part of the Main claim ceiling, not ancillary details to be omitted.
...
The fixed primary matched-background design used:

- non-overlapping pure transition edges;
- non-transition control edges sharing neither endpoint;
- control midpoint within 50 km of the transition midpoint;
- matching on edge length, midpoint elevation and absolute endpoint elevation difference;
- 20 nearest controls, with at least 10 eligible controls required;
- 100,000 random matched-background replicates.

Controls were selected without *Bombus* values. For transition i, matched excess was the transition Hellinger distance minus the median of its fixed matched controls.
...
For the primary 5-km specification, observed mean community turnover was 0.12720 compared with a matched-background mean of 0.11049. The primary P=0.06283 is reported as such and is not relabelled as a significant confirmation. Directional consistency at 10 and 25 km indicates repeated landscape-scale correspondence, but those broader windows are not evidence for the highly local directional colour-maintenance mechanism tested in Main 2.
...
The community result does not strengthen the focal mean into causal evidence because it is unsigned and generated from environment-derived surfaces. The montane result prevents a visually attractive map overlap from being narrated as a second positive mechanism.
