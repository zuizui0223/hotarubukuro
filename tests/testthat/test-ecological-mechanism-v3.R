source(testthat::test_path("..", "..", "scripts", "ecological_analysis_v2.R"))
source(testthat::test_path("..", "..", "scripts", "ecological_mechanism_v3.R"))
...
testthat::test_that("evidence ladder keeps shared and conditional estimands distinct", {
  ...
  design <- readLines(testthat::test_path("..", "..", "MECHANISM_WORKFLOW_V3.md"), warn = FALSE)
  testthat::expect_true(any(grepl("Shared biogeographic structure", design, fixed = TRUE)))
  testthat::expect_true(any(grepl("VIF is a diagnostic", design, fixed = TRUE)))
  testthat::expect_true(any(grepl("prespecified prediction", design, fixed = TRUE)))
  testthat::expect_true(any(grepl("not independent replications", design, fixed = TRUE)))
})
