...
testthat::test_that("result and exclusion registries preserve inference roles", {
  ...
  testthat::expect_true(any(grepl("individual Bombus", exclusions$excluded_item)))
})
...
testthat::test_that("active result registry is finite and directionally coherent", {
  ...
  testthat::expect_gt(result_value(results, "local_bombus_presence", "estimate"), 0)
  testthat::expect_gt(result_value(results, "local_bombus_intensity", "estimate"), 0)
...
testthat::test_that("claim registry preserves causal ceilings", {
  ...
  testthat::expect_match(
    claims$claim_ceiling[claims$claim_id == "C4_local_bombus_turnover"],
    "not causal"
  )
  testthat::expect_match(
